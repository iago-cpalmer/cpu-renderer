...
{
	// v2.y == v3.y
	float m21 = (v2.Position[1] - v1.Position[1]) / (v2.Position[0] - v1.Position[0]);
	float m31 = (v3.Position[1] - v1.Position[1]) / (v3.Position[0] - v1.Position[0]);

	for (int y = floor(v2.Position[1]); y > floor(v1.Position[1]); y--) {

		float xif = (v2.Position[0] + (y - v2.Position[1]) / m21);
		float xff = (v3.Position[0] + (y - v3.Position[1]) / m31);
		...
		float yl = (float)(y - v1.Position[1]) / (v2.Position[1] - v1.Position[1]);

		for (int i = xi; i < xf; i++) {
			float xl = (float)(i - xi) / (float)(xf - xi);

			float xl23 = (float)(i - v2.Position[0]) / (float)(v2.Position[0] - v1.Position[0]);
			float xl13 = (float)(i - v3.Position[0]) / (float)(v3.Position[0] - v1.Position[0]);

			// Interpolate colors along the edges
			COLORREF colorLeft = RGB(
				lerp(v1.Color[0], v2.Color[0], yl),
				lerp(v1.Color[1], v2.Color[1], yl),
				lerp(v1.Color[2], v2.Color[2], yl)
			);
			...
			float uLeft = lerp(v1.UV[0], v2.UV[0], yl);
			float uRight = lerp(v1.UV[0], v3.UV[0], yl);
